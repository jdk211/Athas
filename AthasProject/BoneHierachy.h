...
struct BONE_MESH : public D3DXMESHCONTAINER
{
	...
	std::vector<LPDIRECT3DTEXTURE9>		ReflectionTexs;
...
public:
	void SetLoadPath(std::string filePath);		//로딩 Path 를 정의 해준다.
	void SetSkinnedMesh(cXMesh_Skinned* pSkinnedMesh);		//로딩해야할 스킨드 메시를 정의 한다.



	//STDMETHOD(CreateFrame)(THIS_ LPCSTR Name, LPD3DXFRAME *ppNewFrame) PURE;
	//virtual HRESULT CreateFrame(THIS_ LPCSTR Name, LPD3DXFRAME* ppNewFrame) = 0;


	//
	// 빈 프레임 ( 본노드 ) 할당 ( 본을 생성하는 방법을 정의 한다. )
	//
	...
	// 
	// 빈프레임( 본노드 ) 에 메시를 할당 ( 메시를 할당하는 방법을 정의 한다 )
	//
